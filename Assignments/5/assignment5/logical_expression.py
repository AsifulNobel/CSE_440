...
import logging
logging.basicConfig(filename='logic.log', filemode='w', level=logging.DEBUG)
...
def tt_check_all(KB, alpha, symbols, model):
    alpha_truth_values = []
    var_sym_size = len(symbols)
    tt_string_counter = 0
    tt_string = bin(tt_string_counter)[2:].zfill(var_sym_size)

    logging.debug('model: {}'.format(model))

    while tt_string != ('1' * var_sym_size):
        logging.debug('tt_string: {}'.format(tt_string))

        for index, sym in enumerate(symbols):
            model[sym] = bool(int(tt_string[index]))

        if pl_true(KB, model):
            alpha_truth_values.append(pl_true(alpha, model))

        logging.debug('alpha_truth_values: {}'.format(alpha_truth_values))
...
def check_true_false(KB, alpha):
    ...
    return tt_check_all(KB, alpha, symbol_list, model)
